...
class HashtagTest < ActiveSupport::TestCase
  ...
  test "::trending filters by language" do
    trending_english = Hashtag.trending(limit: 30, language: "en")

    assert_equal 30, trending_english.size

    activity_languages = HashtagActivity.where(hashtag_id: trending_english.map(&:id)).pluck(:language).sort.uniq
